...
- ☁️ 多云资源：切换阿里云 / 腾讯云 / AWS / 华为云，覆盖 VPC / 子网 / 网关 / 弹性公网 IP / 安全组 / 实例 / 路由表。
- 🔗 多 VPC 互联：拖入「对等连接」节点并连接多个 VPC，导出时按两两全互联生成对等连接，并为各 VPC 的路由表自动补全路由。
- 🧩 实例配置：镜像与规格为可编辑下拉（本地内置清单 + 在线清单合并），支持计费方式与密钥对 / 密码登录。
- 🕸️ 隧道网络：宿主机之间隧道互联，自动形成「区域」；逻辑交换机可部署到区域内的所有节点。
- 📤 双格式导出：
  - **OVN**：生成 `ovn-nbctl` / `ovs-vsctl` / `ovn-sbctl` 命令脚本。
  - **Terraform**：按所选厂商生成资源定义（资源引用、下一跳自动解析、系统盘、密钥对资源），并在导出时校验可用区与地域是否匹配。
...
5. 点击顶部「导出 OVN 命令」或「导出 Terraform」查看并复制 / 下载结果。
