...
本仓库内置了一个 Go + Gin 代理实现，见 [`server/`](./server/README.md)：只需在 `server/config.yaml` 填写厂商**只读** AK/SK（或 `mock: true` 无凭证联调），启动后即可接入（腾讯云还会返回实例规格的可用区库存 `zones`）。若使用自建代理，按以下约定接入。
...
工具栏「Provider 版本」用于写入 `provider.tf` 中主 provider 的 `version` 约束（如 `~> 5.0`、`>= 1.200.0`），按云厂商分别保存到本地。

- 可手动输入任意 Terraform 版本约束；也可点击输入框右侧箭头，从**已发布版本下拉**中选择：下拉始终列出全部版本并支持搜索，选中后自动生成 `~> 主.次` 约束（如 `1.98.2` → `~> 1.98`）。
- 下拉候选来自构建时环境变量 `VITE_PROVIDER_VERSIONS_URL`（见 [`.env.example`](./.env.example)），支持 `{vendor}` 占位符逐个厂商请求，也可一次性返回全部厂商；未配置或拉取失败时仅可手输。
- 候选数据由 [`server/`](./server/README.md) 的 `GET /api/providerVersions[/:vendor]` 从 Terraform Registry 拉取（带缓存），也可指向私有 Registry。
